...
class VAE_GRU(nn.Module):
    def __init__(self, input_dim, n_sample=10, window_size=60, z_dim=3, model_dim=500, freeze_layer: str = None):
        super(VAE_GRU, self).__init__()
        self._encoder = Encoder(input_dim, z_dim, model_dim, model_dim)
        self._decoder = Decoder(z_dim, input_dim, model_dim, model_dim)
        ...
        # )
        self.p_z = MultivariateNormal(
            loc=torch.zeros(self.z_dim).to(self._device), 
            covariance_matrix=torch.eye(self.z_dim).to(self._device)
        )
