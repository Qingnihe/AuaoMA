...
def extract_model_params(path):
    params = {}
    params['z1_dim'] = int(re.search(r'z1dim(\d+)', path).group(1)) if re.search(r'z1dim(\d+)', path) else None
    params['model_dim'] = int(re.search(r'modeldim(\d+)', path).group(1)) if re.search(r'modeldim(\d+)', path) else None
    params['prelr'] = float(re.search(r'prelr([0-9e\.-]+)', path).group(1)) if re.search(r'prelr([0-9e\.-]+)', path) else None
...
def load_model(model_path, params, dim):
    model = InterFusion(
        x_dims=dim,
        z1_dims=params['z1_dim'],
        max_epochs=params['epochs'],
        pre_max_epochs=params['epochs'],
